...
/**
 * @brief Funcion ue saca cada uno de los objetos
 */
int SacaObjetoU(SHPO shpo, char *nomarch, FILE *ae, FILE *aex, int objeto, int i_parte, int f_parte);
...
int SacaObjeto(SHPHandle shph, char *nomarch, FILE *ae, FILE *aex, int objeto) {
    ...
    if (shpo->nParts > 0) {
        ...
        for (p = 0; p < shpo->nParts; p++) {
            ...
            SacaObjetoU(shpo, nomarch, ae, aex, objeto, i_parte, f_parte);
        }

    } else {
        SacaObjetoU(shpo, nomarch, ae, aex, objeto, 0, 1);
    }
